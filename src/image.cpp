...
#define MORPH_OP_DEF(func)                                             \
  JNIEXPORT jlong JNICALL IMAGE_FUNC(func)(JNIEnv * env, jclass clazz, \
                                           jlong a, jlong b) {         \
    af_array ret = 0;                                                  \
    AF_CHECK(af_##func(&ret, ARRAY(a), ARRAY(b)));                       \
    return JLONG(ret);                                                 \
  }
...
JNIEXPORT jlong JNICALL IMAGE_FUNC(medfilt)(JNIEnv *env, jclass clazz, jlong a,
                                            jint w, jint h) {
  af_array ret = 0;
  AF_CHECK(af_medfilt(&ret, ARRAY(a), w, h, AF_PAD_ZERO));
  return JLONG(ret);
}
...
JNIEXPORT jlong JNICALL IMAGE_FUNC(bilateral)(JNIEnv *env, jclass clazz,
                                              ...
                                              jfloat color) {
  af_array ret = 0;
  AF_CHECK(af_bilateral(&ret, ARRAY(a), space, color, false));
  return JLONG(ret);
}
...
JNIEXPORT jlong JNICALL IMAGE_FUNC(meanshift)(JNIEnv *env, jclass clazz,
  ...
  af_array ret = 0;
  AF_CHECK(af_mean_shift(&ret, ARRAY(a), space, color, iter, false));
  return JLONG(ret);
}
...
JNIEXPORT jlong JNICALL IMAGE_FUNC(histogram)(JNIEnv *env, jclass clazz,
  ...
  double rmin, imin;
  double rmax, imax;
  AF_CHECK(af_min_all(&rmin, &imin, ARRAY(a)));
  AF_CHECK(af_max_all(&rmax, &imax, ARRAY(a)));

  af_array ret = 0;
  AF_CHECK(af_histogram(&ret, ARRAY(a), nbins, rmin, rmax));
  return JLONG(ret);
}
...
JNIEXPORT jlong JNICALL IMAGE_FUNC(hist_mnmx)(JNIEnv *env, jclass clazz,
                                              ...
                                              jfloat max) {
  af_array ret = 0;
  AF_CHECK(af_histogram(&ret, ARRAY(a), nbins, min, max));
  return JLONG(ret);
}
...
JNIEXPORT jlong JNICALL IMAGE_FUNC(rotate)(JNIEnv *env, jclass clazz, jlong a,
                                           ...
                                           jint method) {
  af_array ret = 0;
  AF_CHECK(af_rotate(&ret, ARRAY(a), theta, crop, (af_interp_type)method));
  return JLONG(ret);
}
...
JNIEXPORT jlong JNICALL IMAGE_FUNC(resize1)(JNIEnv *env, jclass clazz, jlong a,
  ...
  AF_CHECK(af_resize(&ret, ARRAY(a), odim0, odim1, (af_interp_type)method));
  return JLONG(ret);
}
...
JNIEXPORT jlong JNICALL IMAGE_FUNC(resize3)(JNIEnv *env, jclass clazz, jlong a,
                                            ...
                                            jint method) {
  af_array ret = 0;
  AF_CHECK(af_resize(&ret, ARRAY(a), height, width, (af_interp_type)method));
  return JLONG(ret);
}
