...
#define CONVOLVE(N)                                                           \
  JNIEXPORT jlong JNICALL SIGNAL_FUNC(convolve##N)(                           \
    ...
    af_array ret = 0;                                                         \
    AF_TO_JAVA(af_convolve##N(&ret, ARRAY(a), ARRAY(b), (af_conv_mode)method, \
                              AF_CONV_AUTO));                                 \
    return JLONG(ret);                                                        \
  }
...
#define FFT(func, is_inv)                                                      \
  JNIEXPORT jlong JNICALL SIGNAL_FUNC(func)(JNIEnv * env, jclass clazz,        \
    ...
    AF_TO_JAVA(af_##func(&ret, ARRAY(a), norm, dim0));                         \
    return JLONG(ret);                                                         \
  }
...
#define FFT2(func, is_inv)                                                     \
  JNIEXPORT jlong JNICALL SIGNAL_FUNC(func)(                                   \
    ...
    AF_TO_JAVA(af_##func(&ret, ARRAY(a), norm, dim0, dim1));                   \
    return JLONG(ret);                                                         \
  }
...
#define FFT3(func, is_inv)                                                     \
  JNIEXPORT jlong JNICALL SIGNAL_FUNC(func)(JNIEnv * env, jclass clazz,        \
    ...
    AF_TO_JAVA(af_##func(&ret, ARRAY(a), norm, dim0, dim1, dim2));             \
    return JLONG(ret);                                                         \
  }
